class Accident {
    ...
    private onClickHandler(e:JQueryEventObject):void {
        ...
        spinDrone.animate({
            top: this.topPosition - Drone.droneWidth / 2,
            left: this.leftPosition - Drone.droneWidth / 2
        }, 1000, (e => this.onMoveDroneFinish(droneId)));
    }

    private onMoveDroneFinish(droneId:number) {
        // stop the spinning
        // increase the counter
        console.log("moved drone", droneId, "to accident ", this.accidentId);
        DroneContainer.getInstance().getDroneById(droneId).incrementAccidents();
